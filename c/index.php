...
$r = db_row("select id, pagina, bevestigd, persoon from deelnemers where confirmcode='$c' and verwijderd!='j'");
$id = $r[0];
$deelnemer = $r[3];
$pag = $r[1];

$dr = db_row("select voornaam, voorvoegsel,achternaam, email from personen where id='$deelnemer' and verwijderd!='j'");
$naam = $dr[0];
if (!empty($dr[1])) {
    $naam.=" $dr[1]";
}
...
db_query("update deelnemers set bevestigd='j' where confirmcode='$c'");
