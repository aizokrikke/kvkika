	...
	if (empty($err))
	  {
		...
		switch ($do) {
			case 'toevoegen':
				mysql_query("insert into draaiboek (naam, soort, eigenaar, stuurgroep, categorie, prio, status, start, gereed, beschrijving, resultaat, voortgang, benodigdheden, externen, kosten, auteur, aangemaakt, mutatie, mutatie_door) values ('$naam', '$soort', '$eigenaar', '$stuurgroep', '$categorie', '$prio', '$status', '$start', '$gereed', '$beschrijving', '$resultaat', '$voortgang','$materialen', '$externen', '$kosten','$u','$nu','$nu','$u')") or die(mysql_error());
				$id=mysql_insert_id();
			...
			case 'verwijderen':
				mysql_query("update draaiboek set verwijderd='j' where id='$id'") or die(mysql_error());
				$act='';
				$do='';
			break;
		} // switch
...
?>
</select> | status <select name="f_status"><option value="">-- alle --</option><option value="niet gestart" <?php if ($f_status=='niet gestart') { ?> selected="selected" <?php } ?>>niet gestart</option><option value="gestart" <?php if ($f_status=='gestart') { ?> selected="selected" <?php } ?>>gestart</option></select> <input type="checkbox" name="f_gereed" value="j" <?php if ($f_gereed=='j') { ?> checked="checked" <?php } ?>  onClick="form.submit();"> gereed weergeven
</form>
</div>
...
<tr><td></td><td></td><td></td><td></td><td></td><td></td><td>start</td><td></td><td>gereed</td><td></td><td>status</td><td></td><td>eigenaar</td></tr>
<?php
while ($r=mysql_fetch_row($res))
  {
?>
	<tr>
        ...
        <?php 
		  ...
		  {
		?>  
		<td><a href="javascript:void();" alt="<?php echo strip_html(stripslashes($r[4]));?>" title="<?php echo strip_html(stripslashes($r[4]));?>"><?php echo stripslashes($r[1]);?></td>
		<?php
		  }
		?>  
        <td width="10"></td>
		<td><?php if ($r[10]=='voorbereiding') {  echo strftime("%d %B %Y",$r[2]); } else { echo strftime("%d %B %Y %H:%M", $r[2]); }?></td>
        <td width="10"></td>
		 	...
		 	if ($act!='new')
			  {
				if ($opnieuw!='j')  
				  {
					  $r=mysql_fetch_row(mysql_query("select id, naam, eigenaar, stuurgroep, status, prio, categorie, start, gereed, beschrijving, resultaat, voortgang, benodigdheden, kosten, externen, soort, aangemaakt, auteur, mutatie, mutatie_door from draaiboek where id='$id'")) or die(mysql_error());
					  $naam=stripslashes($r[1]);
					  $eigenaar=stripslashes($r[2]);
...
?>    
   ...
   <br>
    ...
    <textarea name="kosten"><?php echo $kosten; ?></textarea>
     ...
     <div class="afsluiten"></div><br>
     
     <input type="submit" name="do" value="<?php if (($act=='new') or ($act=='copy')) { ?>toevoegen<?php } else { ?>opslaan<?php } ?>" class="db_button"> <input type="submit" name="ann" value="annuleren" class="db_button">
     <br /><br /><br />
