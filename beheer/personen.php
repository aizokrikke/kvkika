...
if ($do == 'verwerken') {
    ...
    if (empty($err)) {
      // database updaten
        switch ($action) {
            case 'new':
                db_query("insert into personen 	(voornaam, voorvoegesel, achternaam
                ...
                switch ($call) {
                    case 'beheerders':
                        db_query("insert into beheerders (persoon, actief, verwijderd) values ('$id','j','n'");
                    break;
                } // switch
            break;
            ...
            case 'delete':
                db_query("update personen set verwijderd='j' where id='$id'");
                db_query("update beheerders set verwijderd='j' where persoon='$id'");
            breakk;
        } // switch
        $go = $call;
    }
    ...
    switch ($action) {
        ...
        case 'edit':
            if ($row = db_row("select voornaam, voorvoegsel, achternaam, email, tel, adres, adres_nr, postcode, plaats, login from personen where id='$id'")) {
                $voornamen = $row[0];
                $voorvoegsel = $row[1];
                $achternaam = $row[2];
                $email = $row[3];
