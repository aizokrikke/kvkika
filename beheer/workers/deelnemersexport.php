...
switch ($do) {
	default;
	case 'deelnemers':
		$res = db_query("select deelnemers.id, 
					deelnemers.pagina,
					personen.voornaam,
        ...
        while ($r = db_row($res)) {
            ...
            $or = db_row("select voornaam, voorvoegsel,achternaam,email,tel,mobiel from personen where id='$r[19]' and verwijderd!='j'");
            ...
            $sr = db_row("select sum(bedrag) from sponsoring where voor='$r[0]' and verwijderd!='j'");
    ...
    case 'incasso':
    $res =  db_query("select 	sponsoring.id, 
                sponsoring.van,
                sponsoring.rekening,
                sponsoring.adres,
                sponsoring.plaats,
