...
$r = db_row("select status, lead, body, datum from dev_nieuws where id='$id' and verwijderd!='j'");
if ($r[0] == 'draft') {
 $status = 'public';
} else {
 $status = 'draft';
}
db_query("update dev_nieuws set status='$status' where id='$id'");
// live site bijwerken
if ($rl = db_row("select id from nieuws where id='$id'")) {
  // bericht bestaat al
    db_query("update nieuws set lead='$r[1]', body='$r[2]', status='$status', datum='$r[3]' where id='$id'");
} else {
    db_query("insert into nieuws (id, lead, body, status, datum) values ('$id', '$r[1]', '$r[2]', '$status','$r[3]')");
}
