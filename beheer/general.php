...
switch ($act) {
	...
	case 'edit':
		if ($opnieuw!='j') {
		    $id = $_REQUEST['id'];
			$r= db_row("select id, waarde, soort, beschrijving from system where id='" . db_esc($id) . "' and verwijderd!='j'");
			$waarde = $r[1];
			$soort = $r[2];
			$beschrijving = $r[3];
        }
...
<?php		
		switch ($soort) {
		  case 'jn':
?>
			<td><input type="radio" name="waarde" value="j" <?php if ($waarde=='j') {
			    ?>checked<?php
			} ?> >ja <input type="radio" name="waarde" value="n" <?php if ($waarde=='n') {
			    ?>checked<?php
			} ?> >nee </td>
<?php
		  break;
...
?>
		<table>
<?php        	
		$res = db_query("select id, beschrijving, waarde, soort from system where verwijderd!='j' and editable='j'");
		while ($r = db_row($res)) {
?>
