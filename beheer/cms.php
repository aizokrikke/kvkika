...
$volgorde=mysql_real_escape_string($_REQUEST['volgorde']);
$oldfile=mysql_real_escape_string($_REQUEST['oldfile']);
$live=mysql_real_escape_string($_REQUEST['live']);
$actie=mysql_real_escape_string($_REQUEST['actie']);
$menu_state=mysql_real_escape_string($_REQUEST['menu_state']);
$tekst=mysql_real_escape_string($_REQUEST['tekst']);
$extern=mysql_real_escape_string($_REQUEST['extern']);
...
<?php	
	...
	if ($do=='zet live')
	  {
			...
			mysql_query("update boxen set verwijderd='j' where pagina='".mysql_real_escape_string($id)."'") or die(mysql_error());
			
			// daarna boxen vanuit dev_boxen updaten
			$res=mysql_query("select id,body,datum, verwijderd from dev_boxen where pagina='".mysql_real_escape_string($id)."'");	  
			while ($dbr =mysql_fetch_row($res))
			  {			
			  
			  	//print_r($dbr);	  
				  if ($br=mysql_fetch_row(mysql_query("select id from boxen where id='$dbr[0]'")))
					{ // box bestaat 
					  mysql_query("update boxen set body='$dbr[1]', datum='$dbr[2]', verwijderd='$dbr[3]' where id='$dbr[0]'");
					}
					else
					{
					  mysql_query("insert into boxen (id, body, datum, verwijderd, pagina) values ('$dbr[0]','$dbr[1]','$dbr[2]','$dbr[3]','".mysql_real_escape_string($id)."')") or die(mysql_error());
					}
			  }
	...
	if ($do=='toevoegen')
		{
				  mysql_query("insert into dev_pagina (naam, lead, body, foto, aangemaakt, gewijzigd, auteur, door) values ('$naam','$lead','$body', '$foto','$nu','$nu','$u','$u')") or die(mysql_error());
				  $act='';
		}
...
?>
        ...
        naam<br />
        <input type="text" name="naam" value="<?php echo stripslashes($r[0]); ?>" <?php if (($r[5]=='j') and ($recht_fixed!='j')) { ?> readonly<?php } ?> /><br />
        <br />
        lead<br />
...
<br />
       <?php
		...
		if ($act=='del')  {  if (($r[5]!='j') or ($recht_fixed=='j')) {
			if ($r[5]=='j') 
			  { 
?>
...
?>
			<div class="cms_regel">
			<div class="cms_table_item"><?php if (($r[5]!='j') or ($recht_fixed=='j')) {?><a href="?state=<?php echo $state;?>&go=<?php echo $go;?>&m=<?php echo $m;?>&act=del&id=<?php echo $r[0]; ?>"><img src="beheer/img/24x24/editcut.png" title="verwijderen" alt="verwijderen"></a><?php } else { ?><div style="width:24px;">&nbsp;</div><?php } ?></div>
	...
	case 'logo':
		if (!empty($do))
		  { // formulier afhandelen
			...
			case 'del':
				if ($do=='verwijderen')
				  { mysql_query("update sponsorlogo set verwijderd='j' where id='$id'"); }
				$act='';
				$do='';  
			break;
			...
			case 'edit':
				...
				if (empty($filenaam)) { $filenaam=$oldfile; } // als er geen bestand is geupload...
				mysql_query("update sponsorlogo set naam='$naam', volgorde='$volgorde', file='$filenaam', live='$live' where id='$id'") or die(mysql_error());	
		  		$act='';
				$do='';
			break;
			...
			case 'new':
				...
				if (empty($filenaam)) { $filenaam=$oldfile; } // als er geen bestand is geupload...
				mysql_query("insert into sponsorlogo (naam, volgorde, file, live) values ('$naam', '$volgorde', '$filenaam', '$live')") or die(mysql_error());	
		  		$act='';
				$do='';
			break;
			
			case 'fliplive':
				$r=mysql_fetch_row(mysql_query("select live from sponsorlogo where id='$id'"));
				if ($r[0]=='n') { $live='j'; } else { $live='n'; }
				mysql_query("update sponsorlogo set live='$live' where id='$id'");
				$act='';
				$do='';
			break;	
			}
		...
		switch ($act) {
			case 'del':
				$r=mysql_fetch_row(mysql_query("select id, naam from sponsorlogo where id='$id'"));
?>
...
?>			
                ...
                <table>
                	...
                	<tr><td></td><td width="10"></td><td><?php if(!empty($file)) { ?><img src="https://<?php echo $base_url; ?>/img/logos/<?php echo $file; ?>" width="150" height="69" /><?php } ?></td></tr>
...
<?php
			...
			case 'edit':
				if ($opnieuw!='j') 
				  {
					  $r=mysql_fetch_row(mysql_query("select id, naam, file, volgorde, live from sponsorlogo where id='$id'"));
					  $naam=$r[1];
					  $file=$r[2];
					  $volgorde=$r[3];
					  $live=$r[4];
...
?>
                ...
                <table>
                	...
                	<tr><td></td><td width="10"></td><td><?php if (!empty($file)) { ?><img src="https://<?php echo $base_url; ?>/img/logos/<?php echo $file; ?>" width="150" height="69" /><?php } ?></td></tr>
                    <tr><td>Volgorde</td><td width="10"></td><td><input type="text" name="volgorde" value="<?php echo $volgorde;?>" maxlength="3" size="3" /></td></tr>
                    <tr><td colspan="3"><input type="checkbox" name="live" value="j" <?php if ($live=='j') { ?> checked="checked"<?php } ?> />live </td></tr>
                    <tr><td colspan="3"></td></tr>
                    <tr><td colspan="3" align="right"><input type="submit" name="do" value="bevestigen" /></td></tr>
...
?>
				...
				<table>
<?php			
				$res=mysql_query("select id, naam, volgorde, live from sponsorlogo where verwijderd!='j'");
			 	while ($r=mysql_fetch_row($res))
			   	  {
?>
					<tr>
		...
		if ($do=='bevestigen')
		  {
			  ...
			  if ($extern!='j')
			    { $actie=$menu_state; }
			  mysql_query("update menu set naam='$naam', tekst='$tekst', actie='$actie', extern='$extern' where id='$id'") or die(mysql_error());
			  $act='';
		  }
...
<?php
		switch ($act) {
		  default:
			  $res=mysql_query("select id, naam, tekst from menu where verwijderd!='j' and site='$subdomein' order by id");
			  while ($r=mysql_fetch_row($res))
			    {
?>
			<div class="cms_regel">
...
<?php		
		...
		case 'edit':
				if ($opnieuw!='j') 
				  {
					  $r=mysql_fetch_row(mysql_query("select id, naam, tekst, actie, extern from menu where id='$id'"));
					  $naam=$r[1];
					  $tekst=$r[2];
					  $actie=$r[3];
					  $extern=$r[4];
...
?>
                ...
                <table>
                    ...
                    <tr><td >Actie</td><td></td><td><input type="checkbox" name="extern" value="j" <?php if ($extern=='j') { ?> checked="checked"<?php } ?> onClick="menu_flip_actie()" /> extern </td></tr>
...
<?php
						$sres=mysql_query("select id, state from states where site='www' and verwijderd!='j'");
	...
	case 'foto':
		if (!empty($do))
		  { // formulier afhandelen
			...
			case 'del':
				if ($do=='verwijderen')
				  { mysql_query("update fotos set verwijderd='j' where id='$id'"); }
				$act='';
				$do='';  
			break;
				...
				if (move_uploaded_file($_FILES['bestand']['tmp_name'],$dest))
				  {
					...
					if ($width!='585') { $err[]="Breedte ($width pixels) niet correct (moet 585 pixels zijn)"; }  
					if ($height!='260') { $err[]="Hoogte ($height pixels) niet correct (moet 260 pixels zijn)"; }
					...
					if (empty($err))
					  {  
						...
						if (empty($naam)) { $naam=$filenaam; }
					//if (empty($filenaam)) { $filenaam=$oldfile; } // als er geen bestand is geupload...
						mysql_query("insert into fotos (naam, foto, filenaam, ext) values ('$naam', '$data', '$filenaam','$ext')") or die(mysql_error());	
						$act='';
						$do='';
					  } // if empty
				  } // if move
		...
		switch ($act) {
			case 'del':
				$r=mysql_fetch_row(mysql_query("select id, naam from fotos where id='$id'"));
?>
...
?>
				...
				<table>
<?php			
				$res=mysql_query("select id, naam from fotos where verwijderd!='j'");
			 	while ($r=mysql_fetch_row($res))
			   	  {
?>
					<tr valign="top">
