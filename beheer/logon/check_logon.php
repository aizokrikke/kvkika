...
function login($login,$password,$set_cookie='j') {
  global $user_cookienaam, $user, $hoofddomein;
	...
	if ($row = db_row("SELECT id,voornamen, tussenvoegsel, achternaam, alias FROM personen WHERE login='$login' and password='$password' ORDER BY login")) {
		...
		if ($set_cookie == 'j') { setcookie($user_cookienaam,$login,time()+(60*60*24*365),'/','.'.$hoofddomein);
		} else {
...
//	echo "user_cookie: $user_cookie<br>";
	...
	if (!empty($user_cookie)) {
	    // eerder geaccepteerd -> checken met database
			if ($row = db_row("SELECT id,voornaam, voorvoegsel, achternaam FROM personen WHERE login_md5='$cookie_login_md5' and password=$user_cookie_password ORDER BY login")) {
			    // user bestaat niet in database
				  ...
				  // loggen
				  db_query("INSERT INTO userlog (id, tijd, ip) VALUES ('$row[0]','$tijdstip','$ip')");
...
if ($doe == 'login') { // user login, voor gewone bezoekers!! dus niet beheerders
