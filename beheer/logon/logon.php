...
if (!empty($cookie_login))
	  { // eerder geaccepteerd
	    if (!($row = mysql_fetch_array(mysql_query("SELECT * FROM users WHERE login='$cookie_login' ORDER BY login"))))
		  {  
...
$onderhoud = mysql_fetch_array(mysql_query("SELECT waarde FROM status WHERE parameter='onderhoud_mode'"));
if ($onderhoud[0]=='1')
  { if ($_SESSION['userlevel']<100)
	  { header("Location: https://".$domein."/redactie/onderhoud.php"); }
  }
// loggen
mysql_query("INSERT INTO userlog (user_id, tijd, pagina, ip) VALUES ('$row[0]','$tijdstip','$gekozen_pagina','$ip')")
?>