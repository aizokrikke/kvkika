...
$soort = db_esc($_REQUEST['soort']);
$eigenaar = db_esc($_REQUEST['eigenaar']);
$stuurgroep = db_esc($_REQUEST['stuurgroep']);
$categorie = db_esc($_REQUEST['categorie']);
$status = db_esc($_REQUEST['status']);
$gereed = db_esc($_REQUEST['gereed']);
$prio = db_esc($_REQUEST['prio']);
$db_start = db_esc($_REQUEST['db_start']);
$db_gereed = db_esc($_REQUEST['db_gereed']);
$db_start_uur = db_esc($_REQUEST['db_start_uur']);
$db_start_min = db_esc($_REQUEST['db_start_min']);
$db_eind_uur = db_esc($_REQUEST['db_eind_uur']);
$db_eind_min = db_esc($_REQUEST['db_eind_min']);
$beschrijving = db_esc($_REQUEST['beschrijving']);
$resultaat = db_esc($_REQUEST['resultaat']);
$voortgang = db_esc($_REQUEST['voortgang']);
$materialen = db_esc($_REQUEST['materialen']);
$externen = db_esc($_REQUEST['externen']);
$kosten = db_esc($_REQUEST['kosten']);
...
$br = db_row("select notify_actie_change from beheerders where persoon='$eigenaar'");
$notify = $br[0];
...
if ($ann == 'annuleren') {
	...
	// locks verwijderen
	$lr = db_row("select lock_id from draaiboek where id='$id'");
	if ($u == $lr[0]) {
		db_query("update draaiboek set lock_id='0' where id='$id'");
		db_query("update draaiboek_docs set lock_id='0' where id='$id'");
		db_query("update draaiboek_docs set tmp_verwijderd=verwijderd, tmp_draaiboek=draaiboek where draaiboek='$id'");
    }
}

if (!empty($do)) { // formulier afwikkelen
	
	// fouten afhandelen
	if ($do != 'verwijderen') {
		if (empty($naam)) {
		    $err[] = "geen actie ingevoerd";
		}
    }
	if (empty($err)) {
		switch ($do) {
			case 'toevoegen':
				db_query("insert into draaiboek (naam, soort, eigenaar, stuurgroep, categorie, prio, status, start, gereed, beschrijving, resultaat, voortgang, benodigdheden, externen, kosten, auteur, aangemaakt, mutatie, mutatie_door, lock_id, lock_tijd) values ('$naam', '$soort', '$eigenaar', '$stuurgroep', '$categorie', '$prio', '$status', '$start', '$gereed', '$beschrijving', '$resultaat', '$voortgang','$materialen', '$externen', '$kosten','$u','$nu','$nu','$u','0','0')");
				$id = db_insert_id();
				
				db_query("update draaiboek_docs set verwijderd=tmp_verwijderd, draaiboek='$id', lock_id='0', lock_tijd='' where tmp_draaiboek='1'");
			...
			case 'verwijderen':
				db_query("update draaiboek set verwijderd='j', lock_id='0', lock_tijd='0' where id='$id'");
				$act = '';
				$do = '';
			break;
		} // switch
...
?>
    ...
    $res = db_query("select personen.id, personen.voornaam, personen.voorvoegsel, personen.achternaam from personen, beheerders where beheerders.persoon= personen.id and personen.verwijderd!='j' and beheerders.verwijderd!='j' and beheerders.actief='j' order by achternaam");
    while ($r = db_row($res)) {
    ?>
    ...
    <?php
    $res = db_query("select id, naam from stuurgroepen where verwijderd!='j' order by naam");
    while ($r = db_row($res)) {
    ?>
    ...
    <?php
    $res = db_query("select id, naam from draaiboek_categorie where verwijderd!='j' order by naam");
    while ($r = db_row($res)) {
    ?>
    ...
    ?>
        <tr <?php if ($r[5]=='gereed') {
            ...
            ?>
            <td width="10"></td>
            <td align="center"><?php if ($r[10]=='voorbereiding') {
                    echo strftime("%d %b %Y",$r[2]);
                } else {
                    echo strftime("%d %b %Y<br>%H:%M", $r[2]);
                }?></td>
            <?php
            }
            ?>
  ...
  if ($act=='del') {
      $r = db_row("select id, naam, eigenaar, status, start, gereed, beschrijving from draaiboek where id='$id'");
...
<?php					  
    ...
    if ($act!='new') {
        if ($opnieuw!='j') {
              $r=db_row("select id, naam, eigenaar, stuurgroep, status, prio, categorie, start, gereed, beschrijving, resultaat, voortgang, benodigdheden, kosten, externen, soort, aangemaakt, auteur, mutatie, mutatie_door,lock_id, lock_tijd from draaiboek where id='$id'");
              $naam = stripslashes($r[1]);
              $eigenaar = stripslashes($r[2]);
              ...
              $u = $user['id'];
              $tijd = time();
              db_query("update draaiboek_docs set lock_id='$u', lock_tijd='$tijd', tmp_verwijderd=verwijderd, tmp_draaiboek=draaiboek where draaiboek='$id'");
    ...
    if (!empty($err)) {
          foreach ($err as $val) {
              ?><div class="error">FOUT: <?php echo $val; ?></div><?php
          }
    }
    if ($locked) {
        ?> <div class="error">LET OP: Deze actie is door een andere gebruiker in gebruik. U kunt nu gee wijzigingen opslaan.</div><?php
    }
?>		  
    ...
    draaiboek<br>
    <div class="db_input">
    <input type="radio" name="soort" value="voorbereiding" <?php if ($soort == 'voorbereiding') {
        ?> checked="checked"<?php
    } ?> onClick="flip_draaiboek('voorbereiding');"> voorbereiding <input type="radio" name="soort" value="dag" <?php if ($soort=='dag') { ?> checked="checked"<?php } ?> onClick="flip_draaiboek('dag');"> evenement</div>
	</div>
...
<?php
	$res = db_query("select personen.id, voornaam, voorvoegsel, achternaam from personen, beheerders where beheerders.persoon=personen.id and beheerders.verwijderd!='j' and personen.verwijderd!='j' order by achternaam");
	while ($r = db_row($res)) {
?>
...
<?php
	...
	while ($r = db_row($res)) {
?>
	<option value="<?php echo $r[0]; ?>" <?php if ($r[0] == $stuurgroep) {
...
<?php
	...
	while ($r = db_row($res)) {
?>
	<option value="<?php echo $r[0]; ?>" <?php if ($r[0]==$categorie) {
	    ?> selected="selected" <?php
	    } ?>><?php echo $r[1]; ?></option>
<?php		  
    }
?>    
    ...
    <select name="status" class="db_input">
    	<option value="niet gestart" <?php if ($status == 'niet gestart') {
    	    ?> selected="selected"<?php
    	    } ?>>niet gestart</option>
    	<option value="gestart" <?php if ($status == 'gestart') {
    	    ?> selected="selected"<?php
    	    } ?>>gestart</option>
    	<option value="gereed" <?php if ($status == 'gereed') {
    	    ?> selected="selected"<?php
    	    } ?>>gereed</option>
    </select><br>
   <br>
    ...
    <select name="prio" class="db_input">
    	<option value="showstopper" <?php if ($prio == 'showstopper') {
    	    ?> selected="selected"<?php
    	} ?>>showstopper</option>
    	<option value="must have" <?php if ($prio == 'must have') {
    	    ?> selected="selected"<?php
    	} ?>>must have</option>
    	<option value="nice to have" <?php if ($prio == 'nice to have') {
    	    ?> selected="selected"<?php
    	} ?>>nice to have</option>
    	<option value="laag" <?php if ($prio == 'laag') {
    	    ?> selected="selected"<?php
    	} ?>>laag</option>
    </select>
    </div>
    ...
    <textarea name="kosten"><?php echo $kosten; ?></textarea>
     ...
     <br /><br />
     <input type="submit" name="do" value="<?php if (($act=='new') or ($act=='copy')) {
         ?>toevoegen<?php
     } else {
         ?>opslaan<?php
     } ?>" class="db_button" <?php if ($locked) {
         ?>disabled="disabled"<?php
     } ?>> <input type="submit" name="ann" value="annuleren" class="db_button">
     <br /><br /><br />
